    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#D6D1C7]">
          <h2 className="text-xl font-serif text-[#2C2A26]">Your Reservation ({items.length})</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-[#5D5A53]">Your reservation list is empty.</p>
            </div>
          ) : (
            items.map((item, idx) => (
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-sm font-light text-[#2C2A26]">KSh {item.pricePerDay.toLocaleString()} / day</span>
                    </div>
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          <div className="flex justify-between items-center mb-6">
            <span className="text-sm font-medium uppercase tracking-widest text-[#5D5A53]">Estimated Daily Total</span>
            <span className="text-xl font-serif text-[#2C2A26]">KSh {total.toLocaleString()}</span>
          </div>
          <p className="text-xs text-[#A8A29E] mb-6 text-center">Dates and insurance selected at next step.</p>
          <button
            onClick={onCheckout}
