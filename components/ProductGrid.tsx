...
interface ProductGridProps {
  onProductClick: (product: Car) => void;
}

const ProductGrid: React.FC<ProductGridProps> = ({ onProductClick }) => {
  const { vehicles, loading, error } = useRealtimeVehicles(true); // Only show available vehicles
  ...
  if (loading) {
    return (
      <section id="products" className="py-20 bg-[#F5F2EB]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="text-[#5D5A53]">Loading vehicles...</p>
          </div>
        </div>
      </section>
    );
          ...
          <div className="flex flex-wrap justify-center gap-8 pt-4 border-t border-[#D6D1C7]/50 w-full max-w-2xl">
            {categories.map(cat => (
              <button
                ...
                className={`text-sm uppercase tracking-widest pb-1 border-b transition-all duration-300 ${selectedCategory === cat
